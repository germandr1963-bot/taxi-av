...
app.post('/api/conductor/registro', async (req, res) => {
  try {
    const { nombre, email, telefono, password, vehiculo_marca, vehiculo_modelo, matricula, plazas, isla, numero_taxi } = req.body;
    if (!nombre || !emailValido(email) || !telefono || !password || password.length < 6) {
    ...
    const hash = await bcrypt.hash(password, 10);
    const { rows } = await pool.query(
      `INSERT INTO conductores (nombre, email, telefono, password_hash, vehiculo_marca, vehiculo_modelo, matricula, plazas, isla, numero_taxi)
...
app.get('/api/conductor/me', requiereConductor, async (req, res) => {
  const { rows } = await pool.query(
    `SELECT id, nombre, email, telefono, vehiculo_marca, vehiculo_modelo, matricula, plazas, isla, tipo, estado, disponible, numero_taxi, foto, creado_en
...
app.get('/api/admin/conductores', requiereAdmin, async (req, res) => {
  const { rows } = await pool.query(
    `SELECT id, nombre, email, telefono, vehiculo_marca, vehiculo_modelo, matricula, plazas, isla, tipo, estado, numero_taxi, creado_en
...
app.get('/api/pasajero/viaje-actual', requierePasajero, async (req, res) => {
  const { rows } = await pool.query(
    `SELECT v.*, c.nombre AS conductor_nombre, c.vehiculo_marca, c.vehiculo_modelo, c.matricula,
            c.numero_taxi, c.foto AS conductor_foto,
...
app.post('/api/conductor/foto', requiereConductor, async (req, res) => {
  ...
  await pool.query('UPDATE conductores SET foto = $1 WHERE id = $2', [foto, req.session.usuarioId]);
  res.json({ ok: true });
});
...
app.post('/api/conductor/viaje/:id/aceptar', requiereConductor, conductorAprobado, async (req, res) => {
  ...
  const { rows } = await pool.query(
    `UPDATE viajes SET conductor_id = $1, estado = 'asignado'
